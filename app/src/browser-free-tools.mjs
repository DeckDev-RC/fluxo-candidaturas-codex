...
export const FERRAMENTAS_LIVRES_DE_LEITURA = ['fluxo_browser_observe', 'fluxo_browser_find', 'fluxo_browser_read', 'fluxo_browser_scroll', 'fluxo_browser_back', 'fluxo_browser_navigate', 'fluxo_browser_wait', 'fluxo_browser_screenshot', 'fluxo_browser_hover'];
...
export function browserFreeTools({ browserAdapter, string, opcional }) {
  ...
  return [
    ...
    ['fluxo_browser_find', 'Procurar um texto na página e receber só os elementos que o contêm, com os ancestrais e as refs (ex.: text="Enviar mensagem" devolve o botão dentro do diálogo). Prefira a observe quando você sabe o que procura: gasta uma fração do contexto.', { platform: string, text: string, maxChars: opcional('number') }, (input) => browserAdapter.observe(input.platform, { query: input.text, maxChars: input.maxChars })],
