...
export function createMemoryService({ rootDir, now = () => new Date(), mutationLock = true, lock = () => acquireFluxoLock(rootDir) } = {}) {
  const service = {
    ...
    async safeSummary() {
      ...
      return { facts, selectedResume, preferences: redactValue(memory.preferences), answers: redactValue(memory.answers), lastExecution: memory.executions.at(-1) ?? null, lastUpdated: memory.updatedAt };
    },
    ...
    async upsertFacts(facts = []) {
      ...
      for (const input of Array.isArray(facts) ? facts : []) {
        ...
        memory.facts[key] = {
          ...
          confirmed: input.confirmed !== false, sensitive: input.sensitive === true, createdAt: previous?.createdAt ?? timestamp, updatedAt: timestamp
        };
      }
    ...
    async saveResumeVariant(input = {}) {
      ...
      const value = { id: existing?.id ?? randomUUID(), path, label: String(input.label ?? existing?.label ?? path.split('/').at(-1)), objective: String(input.objective ?? existing?.objective ?? ''), selected: input.selected === true || existing?.selected === true, source: String(input.source ?? existing?.source ?? 'importação local'), updatedAt: timestamp, createdAt: existing?.createdAt ?? timestamp };
  ...
  return wrapMutations(service, ['upsertFacts', 'removeFact', 'saveResumeVariant', 'recordExecution'], { rootDir, mutationLock, lock });
}
...
function normalizeMemory(value) {
  return { version: 1, facts: value?.facts && typeof value.facts === 'object' && !Array.isArray(value.facts) ? value.facts : {}, resumes: Array.isArray(value?.resumes) ? value.resumes : [], answers: value?.answers && typeof value.answers === 'object' ? value.answers : {}, preferences: value?.preferences && typeof value.preferences === 'object' ? value.preferences : {}, executions: Array.isArray(value?.executions) ? value.executions : [], updatedAt: String(value?.updatedAt ?? '') };
}
...
function redactFact(fact) { return { value: clone(fact.value), source: fact.sourceLabel || fact.source, confirmed: fact.confirmed, updatedAt: fact.updatedAt }; }
