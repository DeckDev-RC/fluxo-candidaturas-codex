import { respond, sendJson } from './http-helpers.mjs';
...
export function createSkynetRoutes({ authService = null, conversationService = null } = {}) {
  return {
    ...
    async handle(request, response, { path }) {
      ...
      if (request.method === 'POST' && path === '/api/v1/auth/skynet/login') return respond(response, 202, () => authService.startLogin());
