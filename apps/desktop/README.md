...
Versão: **1.4.1**. Aplicativo local-first com conversa híbrida
SkynetChat/ChatGPT, navegador Playwright embutido e autonomia supervisionada.

Este workspace vive em `apps/desktop/`. O pacote PowerShell legado está em
`packages/powershell/` e a documentação pública na raiz do monorepo.

Abra o instalador `dist/desktop/Fluxo-1.4.1-Windows-x64.exe` ou execute
`npm ci` na raiz do monorepo e `npm run start:desktop`.
[Política](docs/POLITICA-AUTONOMIA.md). [Instalação](docs/DESKTOP.md).
