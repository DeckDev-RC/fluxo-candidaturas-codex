...
// `query` no snapshot e `role`+`name` como alternativa à ref. Ações externas
// recebem approvalId emitido pela interface; o modelo não cria autorizações.
...
export function browserFreeTools({ browserAdapter, string, opcional }) {
  const alvo = { element: opcional('string'), target: opcional('string'), ref: opcional('string'), role: opcional('string'), name: opcional('string'), approvalId: opcional('string') };
  ...
  const agir = (type, extra = () => ({})) => (input, runId) => browserAdapter.act(aba(input), {
    type,
    ref: input.target ?? input.ref,
    role: input.role,
    name: input.name,
    element: input.element,
    ...(['click', 'type', 'press'].includes(type) ? { approvalId: input.approvalId, runId } : {}),
    ...extra(input)
  });
  return [
    ...
    ['browser_click', `Clicar num elemento da página. element: ${DESCRICAO_ELEMENTO} target: ${DESCRICAO_TARGET} Alternativa à ref: role+name (ex.: role=button, name=Mensagem). Ações com efeito fora do app exigem approvalId emitido por Decisões e vinculado à ação exata. Devolve snapshot, changed e diagnostics.`, { platform: plataforma, ...alvo }, agir('click')],
    ['browser_type', `Digitar texto num campo editável; substitui o conteúdo. submit=true também envia Enter e exige approvalId emitido por Decisões. slowly=true digita tecla a tecla. Nunca funciona em senha ou código de verificação.`, { platform: plataforma, ...alvo, text: string, submit: opcional('boolean'), slowly: opcional('boolean') }, agir('type', (input) => ({ text: input.text, submit: input.submit === true, slowly: input.slowly === true }))],
    ...
    ['browser_press_key', 'Pressionar uma tecla. Enter e combinações com Enter exigem approvalId emitido por Decisões; demais teclas permitidas não enviam formulários.', { platform: plataforma, key: string, approvalId: opcional('string') }, agir('press', (input) => ({ key: input.key }))],
