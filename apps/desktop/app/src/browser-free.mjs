...
// Efeito fora do app ou irreversível: exige approvalId emitido pela interface
// e vinculado à página, ao alvo e ao conteúdo exatos.
...
export function createFreeBrowsing({ pageFor, goto, assentar = assentarPadrao, loopGuard = createLoopGuard(), diagnostics = createDiagnostics(), authorizeAction = null }) {
  ...
  async function agir(platform, acao, executar) {
    ...
    try {
      ...
      resultado = await executar(page, async (pagina) => { await diagnostics.assentar(pagina); await assentar(pagina); }, antes);
    } catch (error) {
  ...
  return {
    ...
    async act(platform, acao = {}) {
      ...
      return agir(platform, acao, (page, assentar, fingerprint) => executarAcao(page, platform, acao, tipo, assentar, fingerprint));
    }
  };

  async function executarAcao(page, platform, acao, tipo, assentar, pageFingerprint) {
    if (tipo === 'navigate') {
      ...
      if (/(^|\+)Enter$/.test(tecla)) {
        ...
        const name = await focused.evaluate(nomeAcessivel).catch(() => 'elemento em foco');
        await authorize(page, platform, acao, pageFingerprint, { type: 'press', element: name, key: tecla });
      }
      ...
      try {
        ...
        if (tipo === 'click') {
          ...
          if (ehAcaoSensivel(nome)) await authorize(page, platform, acao, pageFingerprint, { type: 'click', element: nome });
          await elemento.click({ timeout: 10_000 });
        }
        if (tipo === 'type') {
          if (acao.submit === true) {
            const nome = await elemento.evaluate(nomeAcessivel).catch(() => String(resolvido.name ?? 'campo'));
            await authorize(page, platform, acao, pageFingerprint, { type: 'type-and-submit', element: nome, text: String(acao.text ?? '') });
          }
          await digitar(elemento, acao);
        }
        ...
        if (['approval_required', 'policy_gateway_unavailable', 'password_field_forbidden', 'type_not_applied'].includes(error?.code)) throw error;
  ...
  async function authorize(page, platform, action, pageFingerprint, effect) {
    if (typeof authorizeAction !== 'function') throw erro('policy_gateway_unavailable', 'A aprovação autenticada do navegador não está disponível.');
    ...
    return authorizeAction({
      ...
      payload: {
        ...
        page: { url: `${current.origin}${current.pathname}`, fingerprint: pageFingerprint },
        action: effect
      }
    });
  }
