...
const plataforma = (valor) => NOMES[String(valor ?? '').toUpperCase()] ?? (String(valor ?? '').trim() || 'a plataforma em foco');
// O elemento como a IA o descreveu (element, no contrato do Playwright MCP) ou o nome que apontou.
const elemento = (a) => String(a.element ?? a.name ?? '').trim().slice(0, 40);
...
export function resumirFerramenta({ tool, arguments: args = {}, ok, result, error }) {
  ...
  if (ok === false) {
    const approvalId = String(error?.approvalId ?? '');
    return {
      inicio,
      fim: traduzirFalha(tool, args, error),
      espera: error?.code === 'approval_required' && approvalId ? { kind: 'approval', approvalId } : null
    };
  }
...
const FALHAS = {
  ...
  approval_required: () => 'A ação depende da sua aprovação e já aparece em Decisões.',
...
const INICIO = {
  ...
  browser_snapshot: (a) => `Olhando a página de ${plataforma(a.platform)}${a.query ? ` (procurando "${a.query}")` : ''}.`,
  browser_find: (a) => `Procurando "${String(a.text ?? '').slice(0, 40)}" na página de ${plataforma(a.platform)}.`,
  browser_read_text: (a) => `Lendo o conteúdo da página de ${plataforma(a.platform)}.`,
  browser_click: (a) => `Clicando em ${elemento(a) ? `"${elemento(a)}"` : 'um elemento'} da página.`,
  browser_type: (a) => `Digitando "${String(a.text ?? '').slice(0, 40)}"${elemento(a) ? ` em "${elemento(a)}"` : ' na página'}.`,
  browser_select_option: (a) => `Escolhendo "${Array.isArray(a.values) ? a.values[0] ?? '' : a.value ?? ''}"${elemento(a) ? ` em "${elemento(a)}"` : ' na página'}.`,
  browser_console_messages: (a) => `Lendo os erros de console da aba de ${plataforma(a.platform)}.`,
  browser_network_requests: (a) => `Conferindo as requisições que falharam na aba de ${plataforma(a.platform)}.`,
  browser_press_key: (a) => `Pressionando ${a.key ?? 'uma tecla'} na página.`,
  browser_scroll: () => 'Rolando a página.',
  browser_hover: (a) => `Passando o mouse sobre ${elemento(a) ? `"${elemento(a)}"` : 'um elemento'}.`,
  browser_wait_for: (a) => (a.text ? `Esperando "${String(a.text).slice(0, 40)}" aparecer.` : a.textGone ? `Esperando "${String(a.textGone).slice(0, 40)}" sumir.` : 'Aguardando a página.'),
  browser_take_screenshot: () => 'Olhando a tela.',
  browser_navigate: (a) => `Indo para ${enderecoCurto(a.url)} em ${plataforma(a.platform)}.`,
  browser_navigate_back: () => 'Voltando uma página.',
...
const FIM = {
  ...
  browser_snapshot: (a, r) => ({ texto: r.snapshot !== undefined ? `${r.title || enderecoCurto(r.url)}: página observada${r.filtered ? ' (filtrada)' : ''}${r.truncated ? ', cortada' : ''}.` : `${r.title || enderecoCurto(r.url)}: ${Number(r.totalElements ?? 0)} elemento(s) interativo(s).` }),
  browser_find: (a, r) => ({ texto: /nada no snapshot/.test(String(r.snapshot ?? '')) ? `"${String(a.text ?? '').slice(0, 40)}" não está na página.` : `Encontrei "${String(a.text ?? '').slice(0, 40)}" na página.` }),
  browser_hover: () => ({ texto: 'Mouse posicionado.' }),
  browser_wait_for: (a, r) => ({ texto: `Pronto; agora em ${r.title || enderecoCurto(r.url)}.` }),
  browser_take_screenshot: (a, r) => ({ texto: `Vi a tela de ${r.title || enderecoCurto(r.url)}.` }),
  browser_read_text: (a, r) => ({ texto: `Li ${Number(r.text?.length ?? 0)} caracteres de ${r.title || enderecoCurto(r.url)}.` }),
  browser_click: (a, r) => ({ texto: `Cliquei; agora em ${r.title || enderecoCurto(r.url)}.` }),
  browser_type: (a, r) => ({ texto: a.submit ? `Busquei; agora em ${r.title || enderecoCurto(r.url)}.` : 'Texto digitado.' }),
  browser_select_option: () => ({ texto: 'Opção escolhida.' }),
  browser_press_key: () => ({ texto: 'Tecla enviada.' }),
  browser_scroll: (a, r) => ({ texto: `Página rolada (${Number(r.scroll?.y ?? 0)} px).` }),
  browser_navigate: (a, r) => ({ texto: `Aberto: ${r.title || enderecoCurto(r.url)}.` }),
  browser_navigate_back: (a, r) => ({ texto: `De volta a ${r.title || enderecoCurto(r.url)}.` }),
  browser_console_messages: (a, r) => ({ texto: (r.items ?? []).length ? `${(r.items ?? []).length} erro(s) de console.` : 'Sem erros de console.' }),
  browser_network_requests: (a, r) => ({ texto: (r.items ?? []).length ? `${(r.items ?? []).length} requisição(ões) com falha.` : 'Nenhuma requisição falhou.' }),
