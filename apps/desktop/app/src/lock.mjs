...
async function tentarTravar(rootDir) {
  ...
  try {
    ...
    if (error?.code === 'EEXIST') {
      ...
      let ownPid = false;
      try {
        // O caso comum não precisa disputar a trava de recuperação: reconhecer
        // primeiro o próprio PID evita falso "outra execução" sob carga.
        let owner = await lerDono(lockPath);
        ownPid = owner.pid === process.pid;
        if (ownPid) throw lockedError(true);
        await removerRecoveryOrfao(`${lockPath}.recovery`);
        recovery = await open(`${lockPath}.recovery`, 'wx');
        // A trava pode ter mudado enquanto esperávamos o recovery; releia.
        owner = await lerDono(lockPath);
        ownPid = owner.pid === process.pid;
        ...
        if (recoveryError?.code === 'fluxo_locked') throw recoveryError;
