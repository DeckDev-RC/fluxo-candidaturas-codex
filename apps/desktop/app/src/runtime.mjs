...
import { createBrowserActionApproval } from './browser-action-approval.mjs';
...
export async function createLocalRuntime({ rootDir, browserDriver, headless, browserHost = null, skynetHost = null, schedulerTickMs = 60_000 } = {}) {
  ...
  const browserActionApproval = createBrowserActionApproval({ policyGateway });
  const stateStore = createStore({ rootDir, dbPath });
  const driver = browserDriver ?? createPlaywrightDriver({
    rootDir,
    headless: headless ?? runtimeConfig.playwrightHeadless,
    host: browserHost,
    authorizeAction: (input) => browserActionApproval.authorize(input)
  });
