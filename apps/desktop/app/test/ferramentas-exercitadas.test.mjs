...
test('toda ferramenta registrada é exercitada em uma jornada real, inclusive a reconciliação', { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    assert.match((await chamar('browser_read_text', { platform: 'INFOJOBS' }, run.id)).text, /Pessoa Alfa/);
    ...
    await chamar('browser_type', { element: 'campo Pesquisar', target: 'n3', text: 'COBOL', submit: true, approvalId: 'approval-browser' }, run.id);
    ...
    const aceitou = await chamar('browser_click', { platform: 'INFOJOBS', element: 'botão Aceitar', target: 'n2', approvalId: 'approval-browser' }, run.id);
...
function criarQuadro(raiz) {
  ...
  const quadro = {
    ...
    abas: [],
    acoes: [],
    livre: { url: 'https://quadro.test/convites', texto: 'Convites: Pessoa Alfa quer se conectar. Perfil sintético.', digitado: '', elementos: [{ ref: 'n1', role: 'link', name: 'Pessoa Alfa', href: 'https://quadro.test/in/pessoa-alfa', onScreen: true }, { ref: 'n2', role: 'button', name: 'Aceitar', onScreen: true }, { ref: 'n3', role: 'textbox', name: 'Pesquisar', onScreen: true }, { ref: 'n4', role: 'combobox', name: 'Ordenar', options: ['Recentes', 'Antigos'], onScreen: true }] },
