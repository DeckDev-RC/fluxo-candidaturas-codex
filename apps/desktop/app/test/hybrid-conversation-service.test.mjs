...
test('Skynet não recebe mensagens antes do consentimento local', async () => {
  const provider = providerFake('skynet');
  provider.hasConsent = () => false;
  const textual = conversationFake('texto');
  const hybrid = createHybridConversationService({
    textual,
  ...
  await assert.rejects(hybrid.turn('oi'), { code: 'provider_consent_required' });
  assert.equal(textual.calls.length, 0);
});
