...
- [x] Efeito externo: `click` em elemento cujo nome acessível casa com
      `ACAO_SENSIVEL` exige `approvalId` autenticado e vinculado à ação
      (`approval_required`).
...
- [x] Receita para mensagem: abrir compositor → `type` sem submit → solicitar
      aprovação do clique exato → repetir com `approvalId` → `wait` pela
      mensagem na conversa → reportar.
...
- Candidatura em um clique (fixture sintética): sem formulário nem
