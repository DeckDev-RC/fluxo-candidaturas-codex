...
// só com approvalId, ref obsoleta rejeitada com saída por role+name.
...
const PAGINAS = {
  '/convites': `<main><h1>Convites</h1>
    ...
    <ul>
      <li><a href="/in/pessoa-alfa">Pessoa Alfa</a> quer se conectar. <button type="button" onclick="this.closest('li').textContent='Convite sintético aceito'">Aceitar</button></li>
      <li><a href="/in/pessoa-beta">Pessoa Beta</a> quer se conectar. <button type="button">Aceitar</button> <button type="button" id="ignorar-beta" onclick="const n=this.cloneNode(true); n.textContent='Ignorado'; this.replaceWith(n)">Ignorar</button></li>
    </ul>
  ...
  '/in/pessoa-alfa': '<main><h1>Pessoa Alfa</h1><p>Product Engineer em um perfil totalmente sintético.</p></main>',
...
test('navegador da IA: snapshot com refs, ações por ref e por role+name, espera, portões e captura', { timeout: 90_000 }, async (t) => {
  ...
  const driver = createPlaywrightDriver({
    rootDir: await mkdtemp(join(tmpdir(), 'fluxo-livre-')),
    headless: true,
    authorizeAction: ({ approvalId }) => {
      if (approvalId !== 'approval-browser') throw Object.assign(new Error('Aprovação necessária.'), { code: 'approval_required' });
  ...
  assert.match(visto.snapshot, /- listitem \[ref=(?:f\d+)?e\d+\]:\n\s+- link "Pessoa Alfa" \[ref=(?:f\d+)?e\d+\]/, 'a hierarquia diz a quem pertence cada botão');
  ...
  const filtrado = await driver.observe('LINKEDIN', { query: 'Pessoa Alfa' });
  assert.match(filtrado.snapshot, /Pessoa Alfa/);
  assert.doesNotMatch(filtrado.snapshot, /Pessoa Beta/);
  ...
  const aceitarPessoa = refs('Aceitar')[0];
  await assert.rejects(driver.act('LINKEDIN', { type: 'click', ref: aceitarPessoa }), { code: 'approval_required' });
  const depois = await driver.act('LINKEDIN', { type: 'click', ref: aceitarPessoa, runId: 'run-1', approvalId: 'approval-browser' });
  assert.match(depois.snapshot, /Convite sintético aceito/, 'a ação devolve o snapshot novo');
  ...
  await assert.rejects(driver.act('LINKEDIN', { type: 'hover', role: 'link', name: 'Pessoa' }), (erro) => erro.code === 'browser_reference_ambiguous' && /31 elementos "link" casam/.test(erro.message));
  ...
  await assert.rejects(driver.act('LINKEDIN', { type: 'click', role: 'button', name: 'Enviar' }), { code: 'approval_required' });
  const enviado = await driver.act('LINKEDIN', { type: 'click', role: 'button', name: 'Enviar', runId: 'run-1', approvalId: 'approval-browser' });
  ...
  await assert.rejects(driver.act('LINKEDIN', { type: 'type', role: 'textbox', name: 'Pesquisar', text: 'COBOL', submit: true }), { code: 'approval_required' });
  const busca = await driver.act('LINKEDIN', { type: 'type', role: 'textbox', name: 'Pesquisar', text: 'COBOL', submit: true, runId: 'run-1', approvalId: 'approval-browser' });
  ...
  await driver.act('LINKEDIN', { type: 'navigate', url: `${base}/in/pessoa-alfa` });
  assert.match((await driver.readText('LINKEDIN', {})).text, /Product Engineer/);

  // Cena sintética inspirada no layout de mensagem de uma rede profissional.
  await driver.act('LINKEDIN', { type: 'navigate', url: `${base}/in/pessoa-exemplo` });
  ...
  await assert.rejects(driver.act('LINKEDIN', { type: 'click', role: 'button', name: 'Enviar', runId: 'run-1', approvalId: 'approval-browser' }), (erro) => erro.code === 'element_not_actionable' && /desabilitado/.test(erro.message));
  const digitado = await driver.act('LINKEDIN', { type: 'type', role: 'textbox', name: 'Escreva uma mensagem', text: 'Oi, Pessoa Exemplo! Boas-vindas à rede sintética.' });
  ...
  const enviouMsg = await driver.act('LINKEDIN', { type: 'click', role: 'button', name: 'Enviar', runId: 'run-1', approvalId: 'approval-browser' });
  assert.match(enviouMsg.snapshot, /Oi, Pessoa Exemplo! Boas-vindas à rede sintética\./);
