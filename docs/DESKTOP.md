...
Navegador da IA (06/09/2026): além das ferramentas de domínio, a IA opera as abas das plataformas com o método do Playwright MCP — snapshot de acessibilidade com refs (`page.ariaSnapshot({ mode: 'ai' })`), ações por ref (`aria-ref=`) ou por papel e nome, espera por texto, captura de tela entregue ao modelo como imagem (nunca gravada). Portões do produto preservados em `app/src/browser-free.mjs`: senha e código recusados, ação com efeito externo só com confirmação, parada em CAPTCHA/MFA, só URL pública, fronteira de confiança. Detalhes em `docs/CHECKLIST-NAVEGADOR-PLAYWRIGHT.md`.
