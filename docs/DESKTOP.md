...
O driver de produção usa a biblioteca Playwright e uma sessão persistente privada em `estado/browser-profile`. A CLI legada permanece disponível como adaptador. O driver captura URL, campos observados, metadados de vagas e confirmação. Os adaptadores usam JSON-LD JobPosting ou links com empresa observada. Páginas sem estrutura reconhecível param com erro explícito; não há garantia de suporte a cada variante de DOM de cada plataforma.
