...
test('InfoJobs real: cartões com empresa e modalidade, leitor de vaga, candidatura em um clique confirmada', { timeout: 60_000 }, async (t) => {
  const { createServer } = await import('node:http');
  const { mkdtemp } = await import('node:fs/promises');
  const { tmpdir } = await import('node:os');
  const { join } = await import('node:path');
  ...
  const paginas = {
    '/vagas-de-emprego-desenvolvedor-trabalho-home-office.aspx': `<html data-fluxo-plataforma="INFOJOBS"><body>
      <div class="js_rowCard"><a href="https://www.infojobs.com.br/vaga-de-desenvolvedora-react-em-__90000001.aspx"><h2 class="js_vacancyTitle">Desenvolvedor(A) React</h2></a>
      ...
      <div class="js_rowCard"><a href="https://www.infojobs.com.br/vaga-de-analista-em-sao-paulo__11938853.aspx"><h2 class="js_vacancyTitle">Analista Sênior</h2></a>
    ...
    '/vaga-de-desenvolvedora-react-em-__90000001.aspx': `<html data-fluxo-plataforma="INFOJOBS"><body><main>
  ...
  await driver.goto(`${base}/vaga-de-desenvolvedora-react-em-__90000001.aspx`);
  const vaga = await driver.readJobPage();
  ...
  const envio = await adapter.submitWithRetry('submit', { expected: { identifierOrUrl: `${base}/vaga-de-desenvolvedora-react-em-__90000001.aspx` } });
