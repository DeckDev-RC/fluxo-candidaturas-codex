...
**Fechamento da dívida (2026-09-05, segunda rodada):** `http-server.mjs` caiu de 685 para 164 linhas: só transporte (origem local, sessão, trava, registro de operação, arquivos da interface e despacho). As rotas vivem em `app/src/routes/` por grupo (estado, conhecimento, campanha, execução, Codex), cada um com `knows`/`handle`; `fillFromMemory` saiu do transporte e virou `applicationFlow.prepareForReview`. A guarda de composição passou a ser contrato, não teste opcional, e ganhou a contraparte para o risco inverso: `ferramentas-exercitadas.test.mjs` percorre cada ferramenta do registro pela composição real do runtime — inclusive `fluxo_reconcile`, com clique incerto, reconciliação sem novo clique e registro só após confirmação. Ferramenta registrada sem caminho exercitado falha o teste. A medição fica em `npm run test:coverage` (unidade + navegador real sobre `app/src/**`) com piso de 94% linhas, 77% ramos e 88% funções; medido 95,2 / 79,2 / 89,5.
